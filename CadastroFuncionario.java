...
public class CadastroFuncionario extends JFrame implements ActionListener, DocumentListener, ListSelectionListener
{
	...
	// string para replace
	String stringDataDeNascimento, stringCep, stringTelefone, stringCelular, stringSalario;
	
	// variável para tratar salário
	double dSalario;
	...
	JButton bExibirCadastro;
	

	
	JLabel lCodigo, lNome, lDataDeNascimento, lEndereco, lNumero, lBairro, lSexo, lEmail, lCidade, lCep, lUf, 
		lTelefone, lCelular, lSalario, lCargo;
	
	JTextField tCodigo, tNome, tEndereco, tNumero, tBairro, tEmail, tCidade, tSalario, tCargo; 
	...
	CadastroFuncionario()
	{
		...
		setIconImage(new ImageIcon("Imagens/cad_fun.png").getImage());
		...
		lDataDeNascimento.setBounds(625,10,120,20);
		...
		ftDataDeNascimento.setBounds(625,30,70,30);
		...
		tEndereco.setBounds(10,90,390,30);
		p1.add(tEndereco);
		
		// numero
			
		lNumero = new JLabel("Número");
		lNumero.setBounds(410,70,70,20);
		p1.add(lNumero);
				
		tNumero = new JTextField();
		tNumero.setBounds(410,90,70,30);	
		p1.add(tNumero);
		
		// bairro	
		lBairro = new JLabel("Bairro");
		lBairro.setBounds(490,70,100,20);
		p1.add(lBairro);
		
		tBairro = new JTextField();
		tBairro.setBounds(490,90,205,30);
		p1.add(tBairro);
		...
		tCidade.setBounds(10,150,205,30);
		p1.add(tCidade);
		
		// Uf
		
		lUf = new JLabel("UF");
		lUf.setBounds(225,130,100,20);
		p1.add(lUf);
		
		cUf = new JComboBox(estados);
		cUf.setBounds(225,150,170,30);
		p1.add(cUf);
		...
		lCep = new JLabel("Cep");
		lCep.setBounds(405,130,70,20);
		p1.add(lCep);
		
		ftCep= new JFormattedTextField(mfCep);
		ftCep.setBounds(405,150,70,30);
		p1.add(ftCep);
		...
		lTelefone.setBounds(485,130,90,20);
		p1.add(lTelefone);

		ftTelefone = new JFormattedTextField(mfTelefone);
		ftTelefone.setEditable(false);
		ftTelefone.setBounds(485,150,90,30);
		p1.add(ftTelefone);
		
		// celular
		
		lCelular = new JLabel("Celular");
		lCelular.setBounds(595,130,90,20);
		p1.add(lCelular);
		
		ftCelular = new JFormattedTextField(mfCelular);
		ftCelular.setEditable(false);
		ftCelular.setBounds(595	,150,100,30);
		p1.add(ftCelular);
		
		// email
			
		lEmail = new JLabel("Email");
		lEmail.setBounds(10,190,80,20);
		p1.add(lEmail);
		
		tEmail = new JTextField();
		tEmail.setBounds(10,210,310,30);
		p1.add(tEmail);	
		...
		lSalario.setBounds(330,190,100,20);
		p1.add(lSalario);
		
		tSalario = new JTextField(8);
		tSalario.setBounds(330,210,130,30);
		p1.add(tSalario);
		
		// função
		
		lCargo = new JLabel("Cargo");
		lCargo.setBounds(470,190,100,20);
		p1.add(lCargo);
		
		tCargo = new JTextField(8);
		tCargo.setBounds(470,210,225,30);
		p1.add(tCargo);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource() == bSalvar)
		{
			...
			if(index == -1)
			{
				...
				try
				{
				   	...
				   	String sql ="INSERT INTO funcionarios (nome, sexo, data_nascimento," +
					 	"endereco, numero, bairro, cidade, cep, uf, telefone, celular, salario,"+
					 	"cargo, email) Values ('"+
					 	 tNome.getText()+"','"+
					 	 sexo+"','"+
		...
		if(e.getSource() == bExcluir)
		{
			...
			{	
				...
				{
					String sql = "DELETE FROM funcionarios WHERE codigo = '"+tCodigo.getText()+"'";
					int r = statement.executeUpdate(sql);
	...
	*/
	...
	public void setandoEditable(boolean editavel)
	{
	    ...
	    tCargo.setEditable(editavel);
	    tSalario.setEditable(editavel);
	...
	public boolean verificaPreenchimento()
	{
		...
		if(stringDataDeNascimento.equals("00000000"))
		{
		...
		if(tSalario.getText().equals(""))
		{
			JOptionPane.showMessageDialog(null, "Erro!\nPreenchimento do campo salário\nobrigatório",
				"Aviso", JOptionPane.WARNING_MESSAGE);
			tSalario.requestFocus();
			return(false);	
		}
		
		if(tCargo.getText().equals(""))
		{
			JOptionPane.showMessageDialog(null, "Erro!\nPreenchimento do campo cargo\nobrigatório",
				"Aviso", JOptionPane.WARNING_MESSAGE);
			tCargo.requestFocus();
			return(false);	
		}
		...
		//verificando se salário é válido
		stringSalario = tSalario.getText().replace(",",".");
		
		try
		{
			dSalario = Double.parseDouble(stringSalario);
		}
	...
	public void carregarDados()
	{
			...
			try
			{
				resultSet = statement.executeQuery("SELECT * FROM funcionarios WHERE codigo ='"+codigo+"'");
				resultSet.next();
				...
				tSalario.setText(resultSet.getString("salario"));
				tCargo.setText(resultSet.getString("cargo"));
	...
	public void carregaTable()
	{
		try
		{
			resultSet = statement.executeQuery("SELECT count(*) FROM funcionarios");
			...
			{
				resultSet = statement.executeQuery("SELECT * FROM funcionarios LIMIT "+i+",1");
				resultSet.next();
